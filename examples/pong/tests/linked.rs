...
impl Drop for Ticking {
    ...
    fn drop(&mut self) {
        ...
        // From one, not from nought: a `MockNet` seats its peers at
        // `PeerId(1)` up to and including `PeerId(peers())`, and nought is
        // nobody.
        for other in 1..=self.net.peers() {
            if PeerId(other) != me {
    ...
    let handles: Vec<_> = (0..2_u16)
        .map(|seat| {
            let transport = Ticking {
                endpoint: net.endpoint(corvid::peer_of(PlayerId(seat))),
                net: net.clone(),
                period,
            };
