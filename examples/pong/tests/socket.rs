...
/// The seat each end of the link plays.
const HERE: PlayerId = PlayerId(0);

/// And the other one.
const THERE: PlayerId = PlayerId(1);

/// The machine that plays [`HERE`], which is not the same number.
///
/// A seat is counted from nought and a peer from one, because
/// [`PeerId(0)`](PeerId::NONE) is nobody -- so a socket that announced itself
/// as `PeerId(0)` would be reachable by no one, and both ends of this test
/// would wait for a datagram that never comes.
const HERE_PEER: PeerId = corvid::peer_of(HERE);

/// And the machine that plays [`THERE`].
const THERE_PEER: PeerId = corvid::peer_of(THERE);

/// Two sockets on loopback that have found each other.
fn met() -> Result<(UdpNet, UdpNet), Box<dyn std::error::Error>> {
    let here = UdpNet::bind(("127.0.0.1", 0), HERE_PEER)?;
    let there = UdpNet::bind(("127.0.0.1", 0), THERE_PEER)?;
    here.connect(THERE_PEER, there.local()?)?;
    there.connect(HERE_PEER, here.local()?)?;
...
fn two_peers_play_over_loopback() -> Fallible {
    ...
    let mut here = Peer::<Table>::new(Session::new(opening())?, HERE, Budget::DEFAULT);
    let mut there = Peer::<Table>::new(Session::new(opening())?, THERE, Budget::DEFAULT);

    for _ in 0..TICKS {
        step(&mut here, &here_socket, THERE_PEER)?;
        step(&mut there, &there_socket, HERE_PEER)?;
...
fn a_socket_with_nobody_on_it_stalls_rather_than_failing() -> Fallible {
    let socket = UdpNet::bind(("127.0.0.1", 0), HERE_PEER)?;
    socket.connect(THERE_PEER, "127.0.0.1:9")?;

    let mut alone = Peer::<Table>::new(Session::new(opening())?, HERE, Budget::DEFAULT);
    for _ in 0..100 {
        step(&mut alone, &socket, THERE_PEER)?;
    }
