...
// Unconditional, both halves. Every `Controller` declares its input sets and
// is handed an `Input` whether or not there is a device to fill one -- a
// headless run passes an empty snapshot rather than skipping the call -- and
// `Controller::bindings` answers a `Bindings`, which is `platform`'s. What a
// build with no window still does not have is a keyboard to read: `Devices` is
// there and nothing opens one.
