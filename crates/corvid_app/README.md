...
carries is a *name*: the parser holds a string and hands it to
[`Level::load`](corvid_behavior::Level::load), which is the only thing that
knows whether this game has a level by that name.

The name and nothing else, because that is the whole of what `load` takes. So a
game whose levels are self-describing opens on the one named, content and all;
and a game that keeps its levels somewhere a name cannot reach is refused, with
what its own loader said. Those are the two honest answers, and the alternative
to both is a flag that appears to choose the level and only renames it, since
the name is hashed into nothing and the content is what a tick is handed.
