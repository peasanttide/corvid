//! What a run does when a save will not write, or will not read back.
//!
//! The seam against `saves.rs` is success: that file is a save that works, and
//! these are the failures a run has to survive rather than stop for.
