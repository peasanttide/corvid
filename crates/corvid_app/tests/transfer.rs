...
struct Scripted {
    ...
    /// Who is reachable, which never changes for this fixture.
    peers: PeerSet,
}
...
/// The machine that plays a seat, which is not the same number.
///
/// A seat is counted from nought and a peer from one, because `PeerId(0)` is
/// nobody -- so a script that named a seat's number as a peer's would be
/// talking to the machine one along, or to no machine at all.
const fn machine(seat: u16) -> PeerId {
    corvid_app::peer_of(PlayerId(seat))
}
...
fn play(
    ...
    let transport = Scripted::new(&[machine(1 - seat)], incoming, Arc::clone(&recorded));
...
fn a_machine_that_hears_a_stuck_peer_sends_it_a_state() -> Fallible {
    ...
    let (outcome, sent) = play(0, 8, vec![(machine(1), asking, Some(Channel::Control))])?;
...
fn a_state_from_a_seat_that_does_not_answer_is_dropped() -> Fallible {
    ...
    let bytes = corvid_wire::encode(&handed)?;
    let ticks = 8;
    let (outcome, _sent) = play(0, ticks, vec![(machine(1), bytes, Some(Channel::Transfer))])?;
