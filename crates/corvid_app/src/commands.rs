...
pub struct Requests<R> {
    ...
    entries: Vec<Request<R>>,
}

/// An empty record, which is what a run that made no request leaves.
///
/// Hand-written rather than derived, because a derived `Default` would put
/// `R: Default` on the bound and a game's level reference has no reason to have
/// one.
...
impl<R> Requests<R> {
    ...
    pub fn iter(&self) -> impl Iterator<Item = &Request<R>> + '_ {
        self.entries.iter()
    }

    /// Only the ones the runtime had no code for.
    pub fn unhandled(&self) -> impl Iterator<Item = &Request<R>> + '_ {
        self.entries
            .iter()
    ...
    pub fn failed(&self) -> impl Iterator<Item = &Request<R>> + '_ {
        self.entries
            .iter()
...
impl<'a, R> IntoIterator for &'a Requests<R> {
    type Item = &'a Request<R>;
    type IntoIter = std::slice::Iter<'a, Request<R>>;
...
impl<R: fmt::Debug> Sink<R> {
    ...
    pub(crate) fn absorb(&mut self, at: Tick, command: Command<R>, answered: Option<Answer>) {
    ...
    pub(crate) fn into_requests(self) -> Requests<R> {
        self.requests
    }
    ...
    fn act(&mut self, at: Tick, scope: Scope, command: &Command<R>) -> Answer {
        match command {
            Command::Quit(code) => {
