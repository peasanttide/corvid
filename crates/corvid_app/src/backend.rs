...
///
/// Six things travel together -- the tick, the camera, a load in progress, the
/// session's time, the weight between the two extracted states, and what to
/// hear -- and they are a struct because that is what they are: the arguments
/// of a single call that always arrive together and mean nothing apart.
///
/// The renderer is not among them. It is the one thing a frame borrows
/// mutably, and a struct holding it could not be [`Copy`]; it travels as
/// [`present`](Backend::present)'s own argument, which is what leaves this a
/// plain value a helper takes by value.
///
...
#[cfg_attr(
    ...
    expect(
        dead_code,
        reason = "the camera, the load, the time and the weight are read by the screen backend, which is what `render` gates; the headless one takes the same frame and draws none of it"
    )
)]
pub(crate) struct Frame<'a> {
    ...
    /// and a copy of one is smaller than the reference would be.
...
pub(crate) trait Backend<G: Game> {
    ...
    ///
    /// Everything it needs is in the [`Frame`] and in `graphics`, which is the
    /// renderer or [`None`] for a run whose game draws nothing.
    ///
    /// # Errors
    ///
