...
compile_error!(
    "corvid_hash produces a canonical digest only on little-endian targets:      `Hash::hash_slice` absorbs a slice of primitives in the target's own byte      order and cannot be overridden"
);
