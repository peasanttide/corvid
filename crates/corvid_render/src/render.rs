...
///
/// The three references are real `wgpu`. Write buffers through
/// [`queue`](Self::queue), build a texture with [`device`](Self::device) --
/// there is no wrapper in the way of any of it.
///
/// # Why the encoder is not in here
///
/// It is the one thing a frame borrows mutably, and a struct holding it is a
/// struct nothing can copy: every helper a renderer splits its frame across
/// would have to reborrow the whole target to reach the device beside it. So
/// [`Render::draw`] takes the encoder as its own argument and this stays a
/// plain [`Copy`] value that a helper takes by value.
...
/// What a renderer is handed for one frame, beside the encoder it records
/// into.
///
/// One struct rather than five arguments, so that a new thing to hand over is
/// a field here and not a signature change in every implementation. [`Copy`],
...
pub struct Drawing<'a> {
    ...
    ///
    /// Owned rather than borrowed: a [`Camera`] is two fields of fixed point
    /// and copying one is cheaper than the reference would be, and holding a
    /// reference put a lifetime on every renderer that named this type.
    pub camera: Camera,
...
pub trait Render<S: State>: Extract<S> {
    ...
    /// Record one frame.
    ///
    /// `encoder` is already open and is submitted after this returns, so a
    /// frame is recorded rather than submitted here: a windowed renderer
    /// acquires, records, submits and presents in one step, and a frame that
    /// submitted halfway through would be presented halfway through. A game
    /// that genuinely wants its own submission -- a compute prepass whose
    /// results this frame reads back -- has `drawing.target.device` and can
    /// make a second encoder of its own.
    ///
