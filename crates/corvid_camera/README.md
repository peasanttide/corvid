...
here is a fixed-point pose and a frustum, and the 4x4 `f32` matrix a device
...
let ray = camera.ray((Signed16::ZERO, Signed16::ZERO), I16F16::ONE);
...
and a sine and a cosine from `Angle16::sin_cos` miss `sin^2 + cos^2 = 1` by up to
...
and never at the same moment: at 81 degrees/s -- a two-pixel drag -- the two are 138 degrees
