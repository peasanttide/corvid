...
impl Camera {
    ...
    ///
    /// A [`Signed16`] pair, because a screen position is one: its step is
    /// 1/32767 of the viewport, which is finer than a pixel on a display eight
    /// times wider than 4K. The multiplication is
    /// [`I16F16::saturating_mul_signed16`], the same operation an analog axis
    /// is scaled by in `corvid_input` -- a cursor and a stick are the same kind
    /// of number, and this crate does not do its own arithmetic on them.
    ///
    ...
    /// let centre = camera.ray((Signed16::ZERO, Signed16::ZERO), I16F16::ONE);
    ///
    /// // The middle of the screen is straight ahead, which is +Y here.
    /// assert_eq!(centre.direction, Direction::Y);
    /// ```
    #[must_use]
    pub fn ray(&self, ndc: (Signed16, Signed16), aspect: I16F16) -> Ray {
        let pose = self.pose;
        ...
        let up = spread.saturating_mul_signed16(ndc.1);
        let right = spread.saturating_mul(aspect).saturating_mul_signed16(ndc.0);
        ...
        // Clamped, not zeroed. A [`Ray`]'s origin is a `GlobalPoint`, which
        // reaches 8388 km, and a camera further out than that has no origin to
        // give -- but substituting the world origin would answer with a ray
        // that starts at the centre of the planet and looks like a picking bug
        // rather than a range one. `tests/projection.rs` pins it.
        Ray::new(
            pose.position().to_global_saturating(),
