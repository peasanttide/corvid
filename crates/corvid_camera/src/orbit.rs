...
/// `sin² + cos² = 1` by up to 4.3e-5, so 46% of the 65 536 representable yaws
...
/// never at the same moment: at 81°/s — a two-pixel drag — the two are 138°
...
pub struct Orbit {
    ...
    /// what makes [`Camera`](crate::Camera) a trait with two methods on it
    /// instead of two methods and an argument that has to agree with them.
    pub frustum: Frustum,
}

impl Orbit {
    /// A fifth of a turn: 72°, either side of level.
    ///
    ...
    /// the packing's own precision each call — about 0.0034° at worst, which is
...
/// An orbit is a pose and a frustum, which is what a camera is.
/// The eye this orbit camera is looking through.
///
/// Orbit camera is a way of working out where a camera should be; this is the
