...
//!
//! **`analog × dt`** is the bug the split exists to make unwritable: the same
...
fn main() {
    ...
    for (name, model) in [
        ...
        println!("  {name:>11}: {pixels:.0} px turned the camera {total:.3}°");
        println!(
            "               per {:.0} ms: min {low:.4}° median {median:.4}° max {high:.4}°, spread {:.1}x",
            WINDOW * 1000.0,
