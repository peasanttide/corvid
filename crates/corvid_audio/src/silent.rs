...
impl core::fmt::Display for Unavailable {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            Self::NoBackend => {
                f.write_str("this build has no audio backend; the `device` feature adds one")
            }
        }
    }
}
...
impl Audio {
    ...
    #[expect(
        ...
        reason = "the signature mirrors the real `Audio::open`, which consumes the catalogue it plays cues out of; taking it by reference here would make the two builds disagree about ownership"
    )]
