//! What a cue sounds like, and the one voice playing it.
//!
//! There are no recordings to load here, so a sound is described rather than
//! loaded: a [`Timbre`] is four numbers and a [`Voice`] is the oscillator that
//! turns them into samples. That is a stand-in for a catalogue of recordings,
//! and it says so everywhere it is named.

#![allow(
    clippy::suboptimal_flops,
    reason = "the oscillator is written as the two partials summed and then scaled by the envelope, because that is what it is; a fused multiply-add reorders it into something that no longer reads as a waveform, for an accuracy nobody can hear"
)]
