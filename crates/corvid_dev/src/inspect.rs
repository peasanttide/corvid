...
impl Rows {
    ...
    #[allow(
        ...
        reason = "a row's value is usually a `Copy` scalar read straight off the state, and a reference would make every call site spell an `&` for a `u32`"
    )]
...
impl Group<'_> {
    ...
    #[allow(
        clippy::needless_pass_by_value,
        reason = "the same reason `Rows::field` takes its value by value: a row is usually a `Copy` scalar"
    )]
    pub fn field(&mut self, name: &'static str, value: impl fmt::Debug) {
