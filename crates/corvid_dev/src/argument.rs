...
impl fmt::Display for Invalid {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Unknown { path } => write!(f, "no command called {path}"),
            Self::Missing { parameter, of } => {
                write!(f, "{parameter} is required, and is a {of}")
            }
            Self::Extra { words } => write!(f, "{words} words too many"),
            Self::Malformed {
                parameter,
                of,
                given,
            } => write!(f, "{parameter} is a {of}, and {given} is not one"),
            Self::OutOfRange {
                path,
                low,
                high,
                given,
            } => write!(f, "{path} takes {low} to {high}, and not {given}"),
        }
    }
}
