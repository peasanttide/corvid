...
[`const_soft_float`](https://crates.io/crates/const_soft_float). They are
slower than the intrinsic at runtime and identical to it at compile time, which
is the trade this crate is making: the values it is asked for are overwhelmingly
the ones a `const` wants.
