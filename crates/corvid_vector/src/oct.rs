...
//! corners of the square `[-1, 1]^2`. The square is what gets quantized, and it
...
/// | Worst measured error | 0.9569 deg |
/// | Mean measured error | 0.3370 deg over the sphere |
///
...
/// most angle: the half-diagonal of a quantization cell there is `sqrt(1.5)/127`,
/// the radius is `1/sqrt(3)`, and the quotient is `sqrt(4.5)/127` radians -- 0.95703 deg,
/// against 0.956822 deg measured. The README has the seam's own bound and the
...
/// let up = Direction::new(Signed32::ZERO, Signed32::ZERO, Signed32::MAX);
...
impl OctDirection {
    ...
    pub const fn decode(self) -> Direction {
        ...
        match crate::point::normalize_bits([x, y, height], false) {
...
/// direction onto a 127-step grid and costs it up to 0.9569 deg -- a `.into()` on
