...
/// A [`GlobalPoint`](corvid_vector::GlobalPoint) with each component uniform in
/// `-range ..= range`.
pub fn random_global_point(rng: &mut Rng, range: f64) -> corvid_vector::GlobalPoint {
    corvid_vector::GlobalPoint::new(
        corvid_fixed::I24F8::from_f64(rng.next_unit() * range),
        corvid_fixed::I24F8::from_f64(rng.next_unit() * range),
        corvid_fixed::I24F8::from_f64(rng.next_unit() * range),
    )
}

/// A [`GlobalFinePoint`](corvid_vector::GlobalFinePoint) with each component
/// uniform in `-range ..= range`.
pub fn random_global_fine_point(rng: &mut Rng, range: f64) -> corvid_vector::GlobalFinePoint {
    corvid_vector::GlobalFinePoint::new(
        corvid_fixed::I48F16::from_f64(rng.next_unit() * range),
        corvid_fixed::I48F16::from_f64(rng.next_unit() * range),
        corvid_fixed::I48F16::from_f64(rng.next_unit() * range),
    )
}
