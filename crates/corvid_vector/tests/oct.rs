...
//! pass a random-only test while being 90° wrong in a corner. So the sweep walks
...
/// tolerance chosen to pass: the sweep reports 0.956822°, so a change costing a
...
/// the centre of an octahedron face — the `(±1, ±1, ±1)/√3` directions — where
/// the octahedron's surface is nearest the origin, at `1/√3`, so a step of the
...
/// cell sits `(0.5, 0.5, -1)/127` from its corner, a length of `√1.5/127`, all
/// of it perpendicular to the radius. Dividing that by the radius `1/√3` gives
/// `√4.5/127` radians, which is 0.95703°.
///
/// The sweep finds 0.956822°, which is the same to three digits and two
...
/// than at a face centre — 0.643056° measured, against 0.956822° globally.
...
/// direction lies in a coordinate plane. 0.451139° measured.
...
/// straight to `acos` near zero angle reports `√(2e-9)` — 0.003° of pure
...
impl Worst {
    ...
    fn assert_within(&self, limit: f64, what: &str) {
        assert!(
            self.degrees <= limit,
            "{what}: worst {:.6}° over {} samples (limit {limit}°), at {:?}",
            self.degrees,
            self.count,
            self.at
        );
...
fn the_worst_error_over_the_whole_square_is_the_documented_one() {
    ...
    // 1017 × 1017: four samples per encoding step along each axis.
    ...
    assert!(
        ...
        "worst {:.6}° is no longer the documented 0.956822°",
        worst.degrees
    );
    ...
    assert!(
        ...
        "mean {:.6}° is no longer the documented 0.3284°",
        worst.mean()
    );
}
...
fn the_seam_and_the_outer_edges_are_inside_the_same_bound() {
    ...
    assert!(
        seam.degrees > 0.6420,
        "the seam now costs only {:.6}°, so SEAM_DEGREES is no longer its bound",
        seam.degrees
    );
    ...
    assert!(
        ...
        "the outer edges now cost only {:.6}°, so EDGE_DEGREES is no longer their bound",
        edges.degrees
    );
...
fn the_six_axes_survive_exactly_and_the_face_centres_are_the_worst_case() {
    ...
    assert!(
        ...
        "the face centres cost only {:.6}°, so the worst case is not where the \
...
fn a_uniform_sample_of_the_sphere_agrees_with_the_sweep() {
    ...
    assert!(
        ...
        "the sphere-uniform worst {:.6}° is no longer the documented 0.946610°",
        worst.degrees
    );
    assert!(
        (worst.mean() - 0.3370).abs() < 0.0005,
        "the sphere-uniform mean {:.6}° is no longer the documented 0.3370°",
        worst.mean()
    );
...
fn a_decoder_that_clamped_instead_of_folding_would_fail_this_file() {
    ...
    // clamped at zero.
    //
    // It agrees with the real decoder over the whole diamond and is 90° wrong in
    ...
    assert!(
        worst_inside < 1e-4,
        "the two decoders differ by {worst_inside:e}° inside the diamond, where \
         they are supposed to be the same function"
    );
    assert!(
        worst_in_the_corners > 89.0,
        "the clamping decoder was only {worst_in_the_corners:.2}° wrong, so this \
...
fn the_codec_is_within_a_factor_of_two_of_any_sixteen_bit_encoding() {
    ...
    //
    // Sixteen bits name 65536 directions, so each has to stand for `4π/65536`
    // steradians of a sphere that must be covered entirely. A spherical cap of
    // that area has angular radius `r` where `2π(1 - cos r) = 4π/65536`, and a
    ...
    assert!(
        ...
        "the covering floor moved to {floor:.6}°"
    );
