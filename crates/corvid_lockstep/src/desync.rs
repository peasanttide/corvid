...
pub enum Halt {
    ...
    #[error(
        "peer {} sent two different actions for tick {}, so the session has two histories and \
         no way to choose",
        peer.0,
        at.0
    )]
    ...
    /// every caller of it.
    #[error(
        "the level {reference} asked for at tick {} could not be read, and the session cannot \
         advance past a tick whose level is missing",
        at.0
    )]
    Unloadable {
