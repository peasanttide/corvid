...
impl<R> Diverged<R> {
    ...
    pub(crate) fn walked(first: Tick, at: Tick, what: What<R>) -> Self {
        Self {
...
impl<R: fmt::Debug> fmt::Display for Diverged<R> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self.agreed_through {
            Some(through) => write!(
                f,
                "diverged at tick {}, agreed through {through}: {}",
                self.at, self.what,
            ),
            None => write!(
                f,
                "diverged at tick {}, which is the first tick compared: {}",
                self.at, self.what,
            ),
        }
    }
...
impl<R: fmt::Debug + 'static> core::error::Error for Diverged<R> {
...
pub enum What<R> {
    ...
    Actions {
        ...
        /// [`Debug`](fmt::Debug) renders the log's answer — so `None` is a seat
        /// the log has no entry for rather than an action.
        ///
    ...
    Requested {
        ...
        recorded: Option<Box<Request<R>>>,
        /// What the other side asked for.
        computed: Option<Box<Request<R>>>,
    },
...
impl<R: fmt::Debug> fmt::Display for What<R> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Marks { recorded, computed } => write!(
                f,
                "the state was marked {recorded} on the recorded side and \
                 {computed} on the computed one"
            ),
            Self::Actions {
                seat,
                recorded,
                ...
                f,
                "seat {} submitted {recorded} on the recorded side and \
                 {computed} on the computed one",
                seat.0,
            ),
            Self::Reach { recorded, computed } => write!(
                f,
                "the recorded side reached tick {recorded} and the computed one \
                 reached tick {computed}, so one of them stopped early — look at \
                 what the ticks asked for"
            ),
            Self::Requested { recorded, computed } => match (recorded, computed) {
                (Some(recorded), Some(computed)) => write!(
                    f,
                    "the recorded side asked for {:?} and the computed one for {:?}",
                    recorded.command, computed.command,
                ),
                (Some(recorded), None) => write!(
                    f,
                    "the recorded side asked for {:?} and the computed one asked \
                     for nothing more",
                    recorded.command,
                ),
                (None, Some(computed)) => write!(
                    f,
                    "the computed side asked for {:?} and the recorded one asked \
                     for nothing more",
                    computed.command,
                ),
                // Not constructible by anything in this crate: a difference
                // with neither side present is not a difference, so the two
                // comparisons that build this variant only do so when at least
                // one side has a request. Written out rather than left to a
                // catch-all so that a future case cannot land here silently.
                (None, None) => f.write_str(
                    "the two request lists were reported as differing with \
                     nothing on either side, which is a bug in this crate",
                ),
            },
            Self::Unequal { digest } => write!(
                f,
                "the two states compare unequal and digest alike ({digest}): \
                 this game's Eq and Hash disagree, and the digest is the \
...
impl<R> From<Diverged<R>> for Failed<R> {
    fn from(diverged: Diverged<R>) -> Self {
        Self::Diverged(diverged)
    }
}

impl<R: fmt::Debug> fmt::Display for Failed<R> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Refused(why) => write!(f, "the run did not start: {why}"),
            Self::Wrote(why) => write!(f, "the session could not be written down: {why}"),
...
impl<R: fmt::Debug + 'static> core::error::Error for Failed<R> {
    fn source(&self) -> Option<&(dyn core::error::Error + 'static)> {
        match self {
            Self::Refused(why) => Some(why),
            Self::Wrote(why) => Some(why),
            Self::Read(why) => Some(why),
            Self::Diverged(why) => Some(why),
        }
    }
}
