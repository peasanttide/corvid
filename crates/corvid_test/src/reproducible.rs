...
fn play<S, C>(
    ...
    input: Input,
    ticks: u64,
) -> Result<Outcome<Twice<S, C>>, Failed>
where
    S: State + Opens,
    C: corvid_control::Controller<S>,
    C::Config: Default,
...
fn compare<G: Game>(recorded: &Outcome<G>, computed: &Outcome<G>) -> Result<(), Diverged> {
...
/// runs reached.
fn actions<G: Game>(recorded: &Outcome<G>, computed: &Outcome<G>) -> Option<Diverged> {
...
/// against it.
fn requests<G: Game>(recorded: &Outcome<G>, computed: &Outcome<G>, last: Tick) -> Option<Diverged> {
