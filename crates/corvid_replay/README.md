...
**Nothing about the inputs, which is the reason a `PlayerState` has three fields.**
...
| `tests/seek.rs` | That a seek reaches what a hand-written forward run reached, at every budget; that a rollback recovers the forward result, that a stale snapshot is not returned and that ordinary play does not go stale; presence; that a seat no `PlayerId` can name is left out rather than folded onto the last; and that every field of a `PlayerState` is one the capture can rebuild |
