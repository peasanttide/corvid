//! The sessions these tests replay, and a forward run to compare a seek
//! against.
//!
...
//!
//! The game itself is [`super`]; this is everything built out of it.
...
use corvid_behavior::{PlayerId, PlayerState, ProfileId, State as StateContract};
use corvid_hash::{Digest, digest};
use corvid_replay::{ActionLog, HashTrace, Opening, Profile, Schema, Seed, Session};
use corvid_time::Tick;

use super::{Action, Counter, Level, Rules, State, TERMINUS};
        ...
        let players: Vec<PlayerState<Action>> = session
            ...
            .filter_map(|(seat, profile)| {
                ...
                Some(PlayerState {
                    ...
                    action: session.log.get(at, id).copied().unwrap_or(idle),
                })
            })
            .collect();
