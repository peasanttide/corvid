...
/// `(1 - 1/sqrt(2)) * 2^30`: minus the slope of the linear seed over `[1, 2)`.
///
/// Written as exact integer arithmetic rather than a transcribed decimal --
/// `2^30/sqrt(2)` is `sqrt(2^59)`, which [`u64::isqrt`] evaluates at compile time.
...
/// `2(sqrt(2) - 1) * 2^30`: minus the slope of the linear seed over `[0.5, 1)`.
...
/// step takes `e` to about `1.5 e^2`: 4.5e-2 -> 3.1e-3 -> 1.4e-5 -> 2.9e-10. Three
...
///
/// Both operands stay `u64`, so this is a 64x64-to-128 multiply and a shift
...
/// `(1 - 1/sqrt(2)) * 2^15`: minus the slope of the fast seed over `[1, 2)`.
///
/// Exact integer arithmetic for the same reason [`SLOPE_HIGH`] is: `2^15/sqrt(2)` is
/// `sqrt(2^29)`, which [`u32::isqrt`] evaluates at compile time.
...
/// `2(sqrt(2) - 1) * 2^15`: minus the slope of the fast seed over `[0.5, 1)`.
...
/// Newton refines `q` by the residual `1 - n q^2`, and a residual is only as good
/// as the product inside it: `n` and `q^2` each have to fit an operand, their
/// product has to fit a register, so `n` gets about 15 bits and `q^2` about 15,
...
/// spends a 64x64-to-128 multiply on exactly this product -- which is the thing
/// this tier exists to avoid.
///
/// Two steps are therefore enough. The seed holds `|relative error|` under 4.6%,
/// a step takes `e` to about `1.5 e^2`, and 4.6e-2 -> 3.2e-3 -> 1.5e-5 lands under
...
pub(super) const fn rsqrt_fast_bits(x: u32, frac: u32) -> u32 {
    ...
    //
    //    The chord lies above `1/sqrt(n)`, so the seed overestimates and the first
...
/// `(1 - n q^2) * 2^30`, the Newton residual, from a Q15 `n` and a Q15 `q`.
///
