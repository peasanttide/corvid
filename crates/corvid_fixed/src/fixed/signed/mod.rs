...
impl Signed32 {
    /// The value as an [`I16F16`](crate::I16F16).
    ///
    /// Fifteen of the thirty-one bits are dropped, so the result carries a
    /// step of `1/65536` -- two orders of magnitude finer than a pixel on any
    /// display -- and the rounding is to nearest rather than toward zero, so
    /// the error does not accumulate a bias across a screen's width.
    ///
    /// ```
    ...
    pub const fn to_i16f16(self) -> crate::I16F16 {
        // Q31 to Q16 is a fifteen-bit shift, and the half-step added first is
        // what makes it a round rather than a floor. `to_bits` is at most
        // `i32::MAX`, so the shifted sum is at most 65_536 and the narrowing
        // below cannot truncate.
        let scaled = (self.to_bits() as i64 + (1 << 14)) >> 15;
        crate::I16F16::from_bits(scaled as i32)
    }
}
