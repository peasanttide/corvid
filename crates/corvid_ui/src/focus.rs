...
use corvid_shape::{Cast as _, Plane, Ray, project};
...
impl Painted {
    ...
    pub fn panel_to_layout(&self, pose: Transform, metres: I16F16, ray: Ray) -> Option<Position> {
        ...
        let across = project(offset, pose.right());
        let up = project(offset, pose.up());

        // Half the panel, in the ray's own Q8 metres.
        let half_width = i128::from(to_metres(metres).to_bits()) / 2;
        if half_width <= 0 {
            return None;
        }
...
/// A layout-space length as the Q8 metres a raycast works in, rounded to
/// nearest.
const fn to_metres(value: I16F16) -> I24F8 {
    I24F8::from_bits((value.to_bits() + 128) >> 8)
}

/// A wide accumulator brought back to a length, clamping rather than wrapping.
