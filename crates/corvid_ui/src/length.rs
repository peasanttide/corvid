...
//! times the width of an 8K display -- with a 15 upx step, which is three
...
/// [`DEFAULT`](Scale::DEFAULT): one rem is sixteen pixels, unscaled.
///
/// Hand-written because a derived `Default` is zeroes, and a scale of zero
/// collapses every length that goes through it.
...
/// [`AUTO`](Size::AUTO): as large as the content in both directions.
