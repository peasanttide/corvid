//! The reconcile that turns two element trees into as little work as possible.
//!
//! Building is immediate -- a game writes the whole tree out again, which is
//! `element.rs` -- and *reconciliation* is what makes the store retained. Each
//! element carries a digest of its own properties and a digest of its subtree;
//! a subtree whose digest is unchanged is kept whole, including its resolved
//! layout and its focus.
