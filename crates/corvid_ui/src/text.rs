...
#[error("{bytes} bytes of text, and a Line holds {}", Line::CAPACITY)]
pub struct TooLong {
...
/// let long = Line::truncated(&"\u{e9}".repeat(Line::CAPACITY));
...
/// The empty line.
///
/// Hand-written because a derive would zero the length *and* the buffer, and
/// the buffer's contents past the length are not part of the value: keeping
/// one spelling of empty is what makes two empty lines hash alike.
...
/// The text, quoted, as a `&str` prints.
///
/// A derive would print the byte buffer and the length, which is the storage
/// rather than the value -- and the storage is padded, so two equal lines
...
///
/// Hand-written because a derived `Default` is zeroes, and a font whose
/// advance is zero puts every glyph in one place.
