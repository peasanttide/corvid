...
/// The first positional key, which is what an element that was not given one
/// gets before its parent numbers it.
...
/// An empty tree.
///
/// Hand-written because a derive would put `I: Default` on the impl, and an
/// intent type has no reason to have one.
