//! The trait's contract, asserted against every backend this crate carries.
...
use corvid_net::{
    Channel, DATAGRAM_LIMIT, Delivery, Lost, MockNet, PeerId, PeerSet, Schedule, SendError,
    Transport,
};
...
fn a_datagram_reaches_a_connected_peer() {
    ...
    assert_eq!(drain(&bob), [(PeerId(0), Heard::Joined)]);

    alice.send_datagram(PeerId(1), b"tick 41").unwrap();
    net.advance(Duration::from_millis(1));

    assert_eq!(
        drain(&bob),
        [(PeerId(0), Heard::Datagram(b"tick 41".to_vec()))]
    );
}
...
fn a_datagram_to_nobody_is_unknown() {
    ...
    assert_eq!(
        alice.send_datagram(PeerId(0), b"hello"),
...
fn a_datagram_past_the_limit_reports_both_numbers() {
    ...
    alice
        .send_datagram(PeerId(1), &vec![0; DATAGRAM_LIMIT])
        .unwrap();

    assert_eq!(
        alice.send_datagram(PeerId(1), &vec![0; DATAGRAM_LIMIT + 1]),
...
fn a_stream_arrives_in_order() {
    ...
    for frame in 0..6_u8 {
        alice
            .send_stream(PeerId(1), Channel::Control, &[frame])
            .unwrap();
    }
    ...
    let expected: Vec<_> = (0..6_u8)
        .map(|frame| (PeerId(0), Heard::Stream(Channel::Control, vec![frame])))
        .collect();
...
fn a_channel_does_not_hold_up_another() {
    ...
    alice
        .send_stream(PeerId(1), Channel::Chat, b"good luck")
        .unwrap();
...
fn a_join_precedes_and_a_loss_follows() {
    ...
    alice.send_datagram(PeerId(1), b"first").unwrap();
    net.advance(Duration::from_millis(1));
    net.cut(PeerId(0), PeerId(1), Lost::TimedOut);

    assert_eq!(
        drain(&bob),
        [
            (PeerId(0), Heard::Joined),
            (PeerId(0), Heard::Datagram(b"first".to_vec())),
            (PeerId(0), Heard::Lost(Lost::TimedOut)),
        ]
    );
}
...
fn a_roster_is_published_once_per_change() {
    ...
    assert_eq!(roster.as_slice(), [PeerId(1), PeerId(2)]);
    assert!(alice.peers().changed_since(&mut seen).is_none());

    net.cut(PeerId(0), PeerId(2), Lost::Closed);
    let roster = alice.peers().changed_since(&mut seen).unwrap();
    assert_eq!(roster.as_slice(), [PeerId(1)]);
    assert!(alice.peers().changed_since(&mut seen).is_none());
...
fn an_endpoint_for_a_peer_that_is_not_there_reaches_nobody() {
    ...
    assert_eq!(
        nobody.send_datagram(PeerId(0), b"hello"),
        Err(SendError::Unknown(PeerId(0)))
    );
...
fn a_sink_can_answer_from_inside_the_loop() {
    ...
    alice.send_datagram(PeerId(1), b"ping").unwrap();
    net.advance(Duration::from_millis(1));
    ...
    assert_eq!(
        ...
        [
            ...
            (PeerId(1), Heard::Datagram(b"pong".to_vec())),
        ]
    );
}
