...
use corvid_net::{Channel, Delivery, PeerId, Transport, udp::UdpNet};
...
fn joined(here: &UdpNet, there: &UdpNet) -> bool {
    until(here, there, |round| {
        matches!(round, Round::Done)
            && !here.peers().get().is_empty()
            && !there.peers().get().is_empty()
    })
}
...
fn two_sockets_find_each_other() -> Fallible {
    ...
    let met = until(&here, &there, |round| match round {
        ...
        Round::Done => here.peers().get().len() == 1 && there.peers().get().len() == 1,
        Round::Heard(..) => false,
    });

    assert!(met, "two sockets on loopback did not find each other");
    assert!(greetings >= 2, "only {greetings} joins were reported");
    assert!(here.peers().get().contains(PeerId(1)));
    assert!(there.peers().get().contains(PeerId(0)));
    Ok(())
}
...
fn a_peer_nobody_has_heard_from_is_not_reachable() -> Fallible {
    ...
    assert!(here.peers().get().is_empty());
    Ok(())
}
