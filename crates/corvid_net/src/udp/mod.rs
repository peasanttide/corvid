//! A real socket: one `UdpSocket`, real datagrams, and the reliability the
//! trait promises built on top of it.
//!
//! [`MockNet`](crate::MockNet) proves a session survives a link that lies;
//! this proves it survives a link that is *somewhere else*. The runtime above
//! cannot tell them apart, which is the whole reason
//! [`Transport`] is a trait.
//!
//! # What this is
//!
//! One non-blocking [`UdpSocket`], a table of peers by address, and one
//! [`reliable`] channel pair per peer per [`Channel`]. Nothing runs on a thread
//! of its own: [`poll`](Transport::poll) is where packets are read,
//! acknowledgements are sent and retransmissions go out, and a runtime calls it
//! once a tick anyway.
//!
//! # What this is not
//!
//! It is not QUIC, and the difference is worth stating in the negative rather
//! than implying it away.
//!
//! - **No encryption and no authentication.** Anything that can reach the port
//!   can send a packet, and a packet that parses is acted on. What limits the
//!   damage is above: a lockstep peer refuses a datagram naming a tick past its
//!   horizon, drops one it cannot decode, and halts on a contradiction — so the
//!   worst a stranger achieves is the same thing a broken link achieves.
//! - **No congestion control.** [`reliable`] says what that costs.
//! - **No path MTU discovery.** [`DATAGRAM_LIMIT`] is
//!   assumed to fit, which is true of every path that carries 1 280 bytes and
//!   is the conservative internet number.
//! - **No NAT traversal.** Each end is told where the other is.
//!
//! For a game on a local network, or two processes on one machine, that is a
//! transport. For the internet, `quinn` behind this same trait is the answer,
//! and the day it lands nothing above the trait changes.
...
use crate::{
    Channel, DATAGRAM_LIMIT, Delivery, Lost, PeerId, PeerSet, SendError, Transport,
    udp::reliable::{Piece, Receiver, Sender},
};
...
///
/// ```no_run
/// use corvid_net::{PeerId, Transport, udp::UdpNet};
///
...
pub struct UdpNet {
    ...
    /// The peers.
    inner: Mutex<Inner>,
    /// What is published to whoever is watching the roster.
    emitter: Emitter<PeerSet>,
    /// And what they watch.
    peers: Watch<PeerSet>,
}
...
impl UdpNet {
    ...
    pub fn bind(address: impl ToSocketAddrs, me: PeerId) -> io::Result<Self> {
        ...
        let (emitter, peers) = channel("peers", PeerSet::new());
        Ok(Self {
            socket,
    ...
    /// Publishes who is reachable now.
    fn publish(&self) {
        let set: PeerSet = {
            let inner = self.lock();
            inner
