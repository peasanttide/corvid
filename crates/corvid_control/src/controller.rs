...
///
/// Hand-written for the reason the `Copy` below is: a derive goes by which
/// type parameters appear rather than by what the fields hold, so it would
/// put `S: Clone` on the impl for a struct that holds `&S`.
