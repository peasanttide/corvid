...
fn text_is_whatever_the_platform_committed() {
    let mut unspent = Input::new(SETS);
    let mut frame = Input::new(SETS);
    // Three Han characters, spelled as escapes because the workspace keeps its
    // source typeable. What the test needs of them is that they are multi-byte
    // and arrive as one commit, and an escape is the same string.
    let composed = "\u{65e5}\u{672c}\u{8a9e}";
    frame.type_text(composed);
    unspent.absorb(&frame);
    assert_eq!(unspent.text(), composed);
}
