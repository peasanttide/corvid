...
//!
//! Every test here is about the *active set*: which identifiers it owns, and
//! what the ones it does not own read as. The interval between two ticks is
//! `tests/interval.rs`, split off because a file stays under 400 lines.

use corvid_fixed::{I48F16, Signed16};
use corvid_input::{Analog, AnalogId, Digital, DigitalId, IdRange, Input, PoseId, SetId};
