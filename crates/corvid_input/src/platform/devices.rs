...
use crate::snapshot::{Analog, Digital, Input};
...
/// the arithmetic below. The literal cannot be written as a cast in a `const`,
/// so `tests/platform.rs` asserts that a full span of motion produces
/// [`Signed16::MAX`] rather than something one short of it.
const FULL: i64 = 32_767;
...
/// moving it, which is why `tests/platform.rs` asserts both halves.
///
