...
fn the_near_and_far_planes_land_at_zero_and_one() {
    ...
    let near = apply(clip, [0.0, NEAR.to_f64() as f32, 0.0, 1.0]);
    let far = apply(clip, [0.0, FAR.to_f64() as f32, 0.0, 1.0]);
    assert!(
...
fn a_metre_from_the_eye_survives_ten_thousand_kilometres_to_within_a_millimetre() {
    ...
    assert_eq!(eye.coarse, [10_000_000, 0, 1]);
    ...
    let relative = |point: GlobalFinePoint| {
        ...
        [0usize, 1, 2].map(|axis| {
            (axes[axis].to_bits() - (i64::from(eye.coarse[axis]) << I48F16::FRAC_BITS)) as f32
                / 65_536.0
        })
    };
