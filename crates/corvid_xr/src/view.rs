...
#![allow(
    ...
    reason = "this module narrows fixed-point poses into the f32 a device binds, and every cast here is that narrowing; nothing downstream of it is hashed, sent or replayed, so the rounding costs nothing a golden could see"
)]
...
impl Default for EyeView {
    /// At the stage origin, with the roughly 90°-by-90° asymmetric frustum a
...
impl EyeView {
    ...
    /// as the symmetric 45° default's tangent so that a degenerate frustum
    ...
    pub fn eye(self, anchor: Anchor, near: I16F16, far: I16F16) -> Eye {
        ...
        for (axis, component) in world.position().to_array().iter().enumerate() {
            ...
            // for a negative position as well as a positive one.
...
impl Views {
    ...
    pub fn from_head(head: Pose, separation: I16F16, frustum: EyeView) -> Self {
        let half = I48F16::from_bits(i64::from(separation.to_bits()) / 2);
        let offset =
            head.basis()
...
/// How many steps an [`I48F16`] has to the metre.
